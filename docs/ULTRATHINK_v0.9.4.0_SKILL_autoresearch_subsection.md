...
4. **Cross-repo stack**: Perplexity-Tools (orchestrator) → ultrathink-system (reasoning) → ECC Tools → uditgoenka/autoresearch (Claude Code plugin loop) → GPU substrate (optional, via autoresearch_bridge.py for ML experiments).
