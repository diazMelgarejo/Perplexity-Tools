...
python3 ~/.openclaw/scripts/discover.py --status         # show current tier + endpoint health
```
...
1. `~/.openclaw/scripts/discover.py --status` always exits 0 and reports a tier, even if both nodes are down
