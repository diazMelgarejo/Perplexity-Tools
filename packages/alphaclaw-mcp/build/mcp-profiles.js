...
export function toolDisabledMessage(toolName) {
    if (PROCESS_TOOLS.has(toolName)) {
        return (`Tool ${toolName} is disabled (readonly MCP profile). ` +
...
export function profileStartupSummary() {
    ...
    const extras = [];
    if (truthyEnv("ALPHACLAW_MCP_ENABLE_PROCESS_TOOLS"))
        extras.push("process");
    if (truthyEnv("ALPHACLAW_MCP_ENABLE_MUTATING_TOOLS"))
        extras.push("mutating");
