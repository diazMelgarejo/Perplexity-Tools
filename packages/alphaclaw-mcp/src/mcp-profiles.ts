...
export type McpProfileName = "readonly" | "elevated";
...
export const PROCESS_TOOL_NAMES = ["alphaclaw_build_ui", "alphaclaw_run_tests"] as const;

/** Requires elevated profile or ALPHACLAW_MCP_ENABLE_MUTATING_TOOLS=1 */
export const MUTATING_TOOL_NAMES = ["alphaclaw_login", "local_agent_propose_edit"] as const;
...
export type ToolName =
  | (typeof READONLY_TOOL_NAMES)[number]
  | (typeof ELEVATED_ONLY_TOOL_NAMES)[number];
...
export function resolveMcpProfile(): McpProfileName {
  const raw = (process.env.ALPHACLAW_MCP_PROFILE ?? "readonly").trim().toLowerCase();
  if (raw === "elevated" || raw === "full" || raw === "all") return "elevated";
  return "readonly";
}

export function processToolsEnabled(): boolean {
  return resolveMcpProfile() === "elevated" || truthyEnv("ALPHACLAW_MCP_ENABLE_PROCESS_TOOLS");
}

export function mutatingToolsEnabled(): boolean {
  return resolveMcpProfile() === "elevated" || truthyEnv("ALPHACLAW_MCP_ENABLE_MUTATING_TOOLS");
}

export function isToolAllowed(toolName: string): boolean {
  if ((READONLY_TOOL_NAMES as readonly string[]).includes(toolName)) return true;
  if ((PROCESS_TOOL_NAMES as readonly string[]).includes(toolName)) {
    return processToolsEnabled();
  }
  if ((MUTATING_TOOL_NAMES as readonly string[]).includes(toolName)) {
...
export function toolDisabledMessage(toolName: string): string {
  if ((PROCESS_TOOL_NAMES as readonly string[]).includes(toolName)) {
    return (
      `Tool "${toolName}" is disabled in readonly MCP profile. ` +
...
export function profileStartupSummary(): string {
  ...
  return (
    `profile=${profile} process_tools=${proc ? "on" : "off"} ` +
